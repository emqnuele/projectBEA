...
| `context_max_tokens` | `150000` | How much conversation she remembers, in tokens. Slider, 150k–500k, applies without a restart. The recap point and word-for-word present follow it; a bigger memory means a proportionally larger prompt on every turn |
| `handoff_trigger_tokens` | `0` | Manual override: memory size that starts the recap. `0` follows the slider. Must stay above `hot_tokens` |
| `hot_tokens` | `0` | Manual override: the newest tokens that survive a recap exactly as written, never summarized, however old they are. `0` follows the slider |
...
| `minecraft` | `server_url`, `idle_nudge_seconds`, `commentary_seconds`, `build_scripts`, `system_prompt_path` | [minecraft](skills/minecraft.md) |
