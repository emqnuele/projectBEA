...
class AIVtuberBrain:
    ...
    def initialize(self):
        ...
        if not self.png_map:
            print("WARNING: No avatar resources loaded from avatar_map.")

        sys_path = Path(self.config.system_prompt_path)
        try:
            if sys_path.exists():
                self.system_prompt = sys_path.read_text(encoding="utf-8")
                print(f"Loaded system prompt from {sys_path}")
            else:
                print(f"WARNING: System prompt file not found: {sys_path}")
        ...
        print(f"Brain Initialized. Session ID: {self.history_manager.session_id}")
    ...
    def reload_configuration(self):
        ...
        """
        print("--- Hot Reloading Configuration ---")
    ...
    def create_new_session(self):
        ...
        self.history_manager.create_session()
        print(f"Created new session: {self.history_manager.session_id}")
        ...
        try:
            ...
            try:
                idle_path, talking_path = resolve_mood_paths(self.png_map, mood)
                 ...
                 print(f"Could not resolve mood {mood}, checking 'normal'...")
            ...
            async with self.audio_lock:
                ...
                if message:
                    ...
                    try:
                        ...
                        print("Brain: Output tasks cancelled (Interruption).")
                        pass
    ...
    async def interrupt(self):
        ...
        """
        print("Brain: Interruption Signal Received!")
        import sounddevice as sd
        import time
            ...
            try:
                ...
                if consumed_samples < total_samples:
                    ...
                    if len(remaining) > (0.5 * self.playback_sample_rate):
                        ...
                        print(f"Brain: Buffered {len(remaining)/self.playback_sample_rate:.2f}s for resume.")
                    else:
                        self.resume_buffer = None
                else:
                    self.resume_buffer = None
            except Exception as e:
                print(f"Brain: Error calculating resume buffer: {e}")
                self.resume_buffer = None
    ...
    async def _resume_speech(self):
        ...
        if self.resume_buffer is None:
            print("Brain: No resume buffer found.")
            return

        print("Brain: Resuming speech...")
        self.is_speaking = True
        try:
             async with self.audio_lock:
    ...
    async def generate_response(self, user_text: str, system_prompt: str = None) -> Tuple[str, str]:
        ...
        if self.resume_buffer is not None and self._is_backchannel(user_text):
            print(f"Brain: Backchannel detected ('{user_text}'). Resuming...")
    ...
    async def generate_audio_response(self, audio_path: str) -> Tuple[str, str, str]:
        ...
        if self.stt:
             ...
             if transcript:
                  print(f"Brain: Audio Transcript: '{transcript}'")
                  if self.resume_buffer is not None and self._is_backchannel(transcript):
                       print(f"Brain: Audio Backchannel detected. Resuming...")
    ...
    async def process_discord_interaction(self, audio_path: str, username: str) -> Tuple[str, str, str, bytes]:
        ...
        if self.stt:
            transcript = self.stt.transcribe(audio_path)
            print(f"Brain: Transcript from {username}: '{transcript}'")
        ...
        try:
            ...
            print(f"Brain: Error waiting for flush: {e}")
            return "error", "", "", b""
    ...
    async def _flush_buffer(self):
        ...
        print(f"Brain: Flushing {len(items)} items...")

        all_backchannel = all(item['is_backchannel'] for item in items)
        
        if all_backchannel:
             print("Brain: All inputs are backchannels. Resuming.")
             for item in items:
        ...
        async with self.transcript_buffer_lock:
            if self.pending_transcripts:
                ...
                print(f"Brain: Draining {len(self.pending_transcripts)} buffered transcript(s)")
        ...
        print(f"Brain: Combined Context:\n{combined_text.strip()}")
    ...
    async def run_loop(self):
        print("Starting interactive loop. Type 'exit' to quit.")
        print("To send audio, type 'audio:path/to/file.wav'")
        ...
        while True:
            ...
            if user_text.lower().startswith("audio:"):
                ...
                print(f"I will process audio from: {audio_path}")
